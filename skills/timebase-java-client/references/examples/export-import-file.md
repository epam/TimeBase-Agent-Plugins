...
**When to use:** Bulk-exporting a stream to a local file or archive, or importing one back. See `import-export.md` for when this workflow applies.
...
options.setMetaData(set.getContentClasses().length > 1, set); // true = polymorphic
...
MessageWriter2 writer = MessageWriter2.create(file, /* periodicity interval or null */ null, null, descriptors);
...
RecordClassDescriptor findOutType(RecordClassDescriptor fileType) {
    ...
    return null; // no matching type in the target stream — message will be skipped
}
...
try (TickLoader loader = stream.createLoader(loadingOptions);
    ...
    while (reader.next()) {
        ...
        SchemaConverter converter = converters.computeIfAbsent(message.type, fileType -> {
            ...
            if (outType == null) {
                return null; // type not present in target stream; messages of this type are skipped below
            }
