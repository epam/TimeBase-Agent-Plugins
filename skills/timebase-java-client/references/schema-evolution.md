...
To change how existing fields are encoded (e.g. `ENCODING_FIXED_DOUBLE` → `ENCODING_FIXED_FLOAT`), redefine the same fields with the new `DataType` encoding, keeping the same field names and parent, then diff and apply the same way as example 1.
